...
Current live versions: **Lens v4.9 · Cadence v18.10.4 · Deploy v8.18 · Intake v8.10 · FleetConfig v0.1 · numacore_lib v1.5.1**. Most recent build: **Cadence v18.10.4 — the life-of-fleet Budget system**: Generate Budget costs *every* replacement cycle over the horizon (not just next-due) via `benchmark ÷ util_rate`, with a fiscal-anchored monthly→quarterly→annual taper and a cost-weighted **Budget Adjustment Factor** (Fleet→Model→Unit→Component % tree, inherit-with-override, dollar-weighted blend). It produces an **11-sheet Excel** workbook and a self-contained **interactive HTML Budget Report** — four tabs (Parameters · Risk items · Budget graphs · Data table) with a click-through donut, stacked bar, adaptive heat-map, and per-view Excel export. All amounts are today's money (inflation is added by the client on top).
